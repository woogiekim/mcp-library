...
const EXTRACT_SYSTEM = `아래 대화에서 UseCase를 추출하여 반드시 순수 JSON만 응답하세요. 설명 텍스트나 마크다운 없이 JSON만.

응답 형식 (모든 값은 반드시 문자열 또는 숫자 또는 null — 절대 중첩 객체 사용 금지):
{
  "domain": "order",
  "title": "UseCase 제목",
  "version": "1.0.0",
  "scenarios": [{"stepOrder": 1, "description": "단계 설명 문자열", "expected": "기대 결과 문자열 또는 null"}],
  "rules": [{"description": "규칙 설명 문자열", "constraint": "제약조건을 한 줄 문자열로 표현 예: amount >= 1000 && period <= 30"}],
  "exceptions": [{"condition": "예외 조건 문자열", "handling": "처리 방법 문자열"}]
}

주의:
- constraint는 반드시 문자열 한 줄. 절대 객체나 배열 사용 금지.
- scenarios 최소 2개, rules 최소 1개, exceptions 없으면 빈 배열 [].`
